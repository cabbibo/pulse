function City( batteryPosition , x ){
	...
	for( var i = 0 ; i < 4; i++ ){
		...
		finalC.push( new THREE.Vector3(35 , 0 , -2.1  * ((4-i)+1)));
		...
		for( var j = 0; j < 4; j++ ){
			...
			var connected = this.createChunk( full , start , end , j + i*4 );//new PathConnector( connectionPaths , finalConnect , this.xWidth , .1 , 0 )
...
City.createRow = function( rowLength , basePos , rowDir , endOffset , wireOffset ){
	...
	var rowTan = rowDir.clone();
	rowTan.applyAxisAngle( this.upVec , Math.PI / 2 )
