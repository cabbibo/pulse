function City( batteryPosition , x ){
	...
	this.spaceBetweenChunks = this.spaceBetweenBuildings * this.buildingsInRow * 2.;
	this.chunksInChunkRow = 8;

	this.spaceBetweenChunkRows = 1.//this.buildingsInRow * this.rowsInChunk * this.chunksInChunkRow * this.sbw * 4
	this.chunksInChunkChunk = 8 ;
...
City.prototype.createCity = function(){
	...
	for( var  i = 0; i < 6; i++){
		...
		var basePos = new THREE.Vector3( .2 , 0 , 4 + i * 10 );
		var rowDir = new THREE.Vector3( 1, 0 , 0  );
		var endOffset = this.buildingSize * 4;
		var wireOffset = this.buildingSize * 4;
